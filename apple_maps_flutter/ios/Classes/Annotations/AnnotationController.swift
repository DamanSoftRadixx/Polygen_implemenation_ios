...
extension AppleMapController: AnnotationDelegate {
  ...
  public func mapView(_ mapView: MKMapView, annotationView view: MKAnnotationView, didChange newState: MKAnnotationView.DragState, fromOldState oldState: MKAnnotationView.DragState) {
     if newState == .ending {
      if let annotation = view.annotation as? FlutterAnnotation {
       let newCoordinate = annotation.coordinate
        self.channel.invokeMethod("annotation#onDragEnd", arguments: ["annotationId": annotation.id,
         "position": [newCoordinate.latitude, newCoordinate.longitude]])
      }
     }else if newState == .starting {
       if let annotation = view.annotation as? FlutterAnnotation {
        let newCoordinate = annotation.coordinate
         self.channel.invokeMethod("annotation#onDragStart", arguments: ["annotationId": annotation.id,
          "position": [newCoordinate.latitude, newCoordinate.longitude]])
       }
      }
    }
    ...
    public func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
        if let annotation: FlutterAnnotation = view.annotation as? FlutterAnnotation {
            ...
            // Do not trigger tap event if the annotation was dragged
            if annotation.wasDragged {
                annotation.wasDragged = false
                return
            }
    ...
    func getAnnotationView(annotation: FlutterAnnotation) -> MKAnnotationView {
        ...
        if annotationView!.isDraggable == true {
