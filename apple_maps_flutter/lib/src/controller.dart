...
class AppleMapController {
  ...
  Future<dynamic> _handleMethodCall(MethodCall call) async {
    switch (call.method) {
      ...
      case 'annotation#onDragEnd':
        print('_handleMethodCall end${call.method}');

        _appleMapState.onAnnotationDragEnd(call.arguments['annotationId'],
            LatLng._fromJson(call.arguments['position'])!);
        break;
      case 'annotation#onDragStart':
        print('_handleMethodCall start${call.method}');

        _appleMapState.onAnnotationDragStart(call.arguments['annotationId'],
            LatLng._fromJson(call.arguments['position'])!);
        break;
  ...
  /// Return [LatLngBounds] defining the region that is visible in a map.
  Future<void> onClickTempTap() async {
    print('inside on Tap evint hit');
    _appleMapState.onTap(const LatLng(0.0, 0.0));

    print('after on Tap evint hit');
  }
